...
checkLaterEqual <- function(parameter,value) {
  name = deparse(substitute(parameter))
  if(class(parameter)!="Date") parameter = as.Date(parameter)
  if(class(value)!="Date") value = as.Date(parameter)
  if (parameter<value) {
    ParallelLogger::logError(paste0(name, ' needs to later than ',value))      
    stop(paste0(name, ' needs to be >= ', value))
  }
  return(TRUE)
}
