...
{DEFAULT @cohort_table = 'observation_period' }
...
{@use_observation_period} ? {
FROM ( -- raw_cohorts 
       SELECT person_id AS subject_id,
       @cohort_id AS cohort_definition_id, --Should I use @cohort_id rather than 0???
       {@study_start_date != '' } ? { 
        CASE 
       ...
       FROM @cdm_database_schema.observation_period AS cohort_table
       {@study_end_date != '' } ? { 
        WHERE observation_period_start_date <= CAST('@study_end_date' AS DATE)
          --{@study_start_date != '' }? should we restrict cohort to those have been observed before study_start_date? 
        }
) raw_cohorts
}:{
FROM ( -- raw_cohorts 
