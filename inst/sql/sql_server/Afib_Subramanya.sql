...
WHERE (C.condition_start_date >= DATEFROMPARTS(2007, 01, 01) and C.condition_start_date <= DATEFROMPARTS(2015, 10, 01))
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,7,P.START_DATE) AND A.START_DATE <= DATEADD(day,365,P.START_DATE)
GROUP BY p.person_id, p.event_id
HAVING COUNT(A.TARGET_CONCEPT_ID) >= 1
-- End Correlated Criteria
